...
void querier_setup(const char* addr, int port)
{
    ...
    // NOTE: restart enclave to destory all previous states
    drop_enclave();
    if (load_enclaves() != SGX_SUCCESS)
        printf("\nLoad Enclave Failure");

//    ecall_querier_setup(b_index_input, b_index_input_size);
    status = Enclave2_ecall_querier_setup(e2_enclave_id,
        b_index_input, b_index_input_size
        );
    if (status != SGX_SUCCESS) {
        printf("Enclave2_ecall_querier_setup failed: Error code is %x\n", status);
    }
...
//    ecall_do_query(request, request_size, response, response_size);
    status = Enclave2_ecall_do_query(e2_enclave_id,
        request, request_size,
        response, response_size
        );
    if (status != SGX_SUCCESS) {
        printf("Enclave2_ecall_do_query failed: Error code is %x\n", status);
    }
}

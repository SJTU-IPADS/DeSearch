...
void witness_input(witness_context &context, const void *start, size_t size) // append an input
{
    ...
    sgx_sha256_msg((const uint8_t *)start, size, (sgx_sha256_hash_t*)&output_hash[0]);
    sgx_sha256_string(output_hash, _in);
...
void witness_output(witness_context &context, const void *start, size_t size) // append an output
{
    ...
    uint8_t output_hash[SGX_SHA256_HASH_SIZE];
    sgx_sha256_msg((const uint8_t *)start, size, (sgx_sha256_hash_t*)&output_hash[0]);
    sgx_sha256_string(output_hash, _out);
    context.out = _out;
    context.stage = 3;
}
...
void witness_func(witness_context &context, const void *start, size_t size) // const void (*func)(void)
{
    ...
    uint8_t output_hash[SGX_SHA256_HASH_SIZE];
    sgx_sha256_msg((const uint8_t *)start, size, (sgx_sha256_hash_t*)&output_hash[0]);
    sgx_sha256_string(output_hash, _bin);
    context.bin = _bin;
    context.stage = 4;
}

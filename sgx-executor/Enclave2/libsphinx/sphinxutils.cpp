...
bool CSphConfigParser::ValidateKey ( const char * sKey )
{
	...
	if ( pDesc->m_iFlags & KEY_DEPRECATED )
		if ( ++m_iWarnings<=WARNS_THRESH )
			printf ( "WARNING: key '%s' is deprecated in %s line %d; use '%s' instead.\n",
	...
	if (!( pDesc->m_iFlags & KEY_LIST ))
	{
		...
		if ( tSec(sKey) && !tSec[sKey].m_bTag )
			if ( ++m_iWarnings<=WARNS_THRESH )
				printf ( "WARNING: key '%s' is not multi-value; value in %s line %d will be ignored.\n", sKey, m_sFileName.cstr(), m_iLine );
	}

	if ( pDesc->m_iFlags & KEY_REMOVED )
		if ( ++m_iWarnings<=WARNS_THRESH )
			printf ( "WARNING: key '%s' was permanently removed from Sphinx configuration. Refer to documentation for details.\n", sKey );
...
bool CSphConfigParser::Parse ( const char * sFileName, const char * pBuffer )
{
	...
	if ( m_iWarnings>WARNS_THRESH )
		printf ( "WARNING: %d more warnings skipped.\n", m_iWarnings-WARNS_THRESH );
...
void sphConfDictionary ( const CSphConfigSection & hIndex, CSphDictSettings & tSettings )
{
	...
	if ( hIndex("dict") )
	{
		...
		if ( hIndex["dict"]=="crc" )
		{
			...
			printf ( "WARNING: unknown dict=%s, defaulting to keywords\n", hIndex["dict"].cstr() );
	}
}
...
bool sphConfIndex ( const CSphConfigSection & hIndex, CSphIndexSettings & tSettings, CSphString & sError )
{
	...
	{
		printf ( "WARNING: min_prefix_len=0, prefix_fields ignored\n" );
		tSettings.m_dPrefixFields.Reset();
	}

	if ( tSettings.m_iMinInfixLen==0 && tSettings.m_dInfixFields.GetLength()!=0 )
	{
		printf ( "WARNING: min_infix_len=0, infix_fields ignored\n" );
	...
	if ( hIndex("docinfo") )
	{
		if ( hIndex["docinfo"]=="none" )		tSettings.m_eDocinfo = SPH_DOCINFO_NONE;
			...
			printf ( "WARNING: unknown docinfo=%s, defaulting to extern\n", hIndex["docinfo"].cstr() );

		if ( tSettings.m_eDocinfo==SPH_DOCINFO_INLINE )
			printf ( "WARNING: docinfo=inline is deprecated, use ondisk_attrs=1 instead\n" );
	...
	if ( hIndex("hit_format") )
	{
		if ( hIndex["hit_format"]=="plain" )		tSettings.m_eHitFormat = SPH_HIT_FORMAT_PLAIN;
			...
			printf ( "WARNING: unknown hit_format=%s, defaulting to inline\n", hIndex["hit_format"].cstr() );
	}
	...
	{
		printf ( "WARNING: both rlp_chinese and rlp_chinese_batched options specified; switching to rlp_chinese\n" );
		bBatchedRLP = false;
	}
...
bool sphFixupIndexSettings ( CSphIndex * pIndex, const CSphConfigSection & hIndex, CSphString & sError, bool bTemplateDict )
{
	...
	if ( tSettings.m_bIndexExactWords && !bNeedExact )
	{
		...
		pIndex->Setup ( tSettings );
		printf ( "WARNING: no morphology, index_exact_words=1 has no effect, ignoring\n" );
	}
		...
		( tSettings.m_iMinPrefixLen || tSettings.m_iMinInfixLen ) && !tSettings.m_bIndexExactWords )
	{
		...
		pIndex->Setup ( tSettings );
		printf ( "WARNING: dict=keywords and prefixes and morphology enabled, forcing index_exact_words=1\n" );
	}
...
static void StdoutLogger ( ESphLogLevel eLevel, const char * sFmt, va_list ap )
{
	...
	switch ( eLevel )
	{
	case SPH_LOG_FATAL: printf ( "FATAL: " ); break;
	case SPH_LOG_WARNING: printf ( "WARNING: " ); break;
	case SPH_LOG_INFO: printf ( "WARNING: " ); break;
	...
	case SPH_LOG_VERY_VERBOSE_DEBUG: printf ( "DEBUG: " ); break;
	}
